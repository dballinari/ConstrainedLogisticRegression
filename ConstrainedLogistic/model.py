...
class ConstrainedLogisticRegression:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray, bounds: List[Tuple[float, float]]=None):
        """
        ...
        coef_fit = scipy.optimize.minimize(self._objective, x0=np.zeros(self._p), args=(x, y, ), jac=self._jac, method='L-BFGS-B', bounds=bounds).x
...
# def fit(x: np.ndarray, y: np.ndarray, include_intercept: bool=False, bounds: List[Tuple[float, float]]=None) -> Tuple[np.ndarray, float]:
#     """
#     Fit a logistic regression model with constraints on the coefficients.
...
#         float: The intercept of the logistic regression model.
...
#     # add per default unconstrained intercept
#     if bounds is not None and include_intercept and len(bounds) == p - 1:
#         bounds = [(None, None)] + bounds
...
#     coef_fit = scipy.optimize.minimize(objective, x0=np.zeros(p), args=(x, ), jac=jac, method='L-BFGS-B', bounds=bounds).x
    
#     if include_intercept:
#         intercept = coef_fit[0]
#         coef_fit = coef_fit[1:]
#     else:
#         intercept = 0
#     return coef_fit, intercept


# def predict(x: np.ndarray, coef: np.ndarray, intercept: float=0) -> np.ndarray:
    # """
    # Predict the response variable given the predictors and coefficients.
    
    # Parameters:
    #     x (np.ndarray): predictors with shape (number observations, number features).
    #     coef (np.ndarray): The coefficients of the logistic regression model.
